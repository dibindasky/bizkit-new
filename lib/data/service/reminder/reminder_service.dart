...
class ReminderService implements ReminderRepo{
  ...
  @override
  Future<Either<Failure, SuccessResponseModel>> createReminder({required CreateReminderModel createReminderModel}) async{
    try {
      log('createReminder ');
      ...
      return Right(SuccessResponseModel(message: 'Reminder added successfully'));
    } on DioException catch (e) {
      log('createReminder dio error');
      log(e.toString());
      log(e.response.toString());
      return Left(Failure(message: 'Failed to create reminder please try again'));
    } catch (e) {
  ...
  @override
  Future<Either<Failure, SuccessResponseModel>> editReminder({required CreateReminderModel createReminderModel}) async{
        try {
      log('createReminder ');
      print('createReminder  ${createReminderModel.toJson()}');
      await _apiService.patch(
          ApiEndPoints.editReminder.replaceFirst('{reminder_id}', createReminderModel.id!.toString()),
          data: createReminderModel.toJson());
      log('createReminder done');
      return Right(SuccessResponseModel(message: 'Reminder added successfully'));
    } on DioException catch (e) {
